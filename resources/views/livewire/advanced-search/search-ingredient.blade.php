<div>
    <div class="form-group relative w-50 max-h-36 mx-auto z-10">
        <input type="text" id="searchInput" class="z-20 form-control rounded text-center"
        ...
        <div class="absolute top-3 right-3 spinner-grow spinner-grow-sm text-primary" wire:loading role="status">
            <span class="sr-only">Loading...</span>
        </div>
        @if ($ingredients)
            <div class="fixed top-0 right-0 left-0 bottom-0" wire:click='resetData'></div>
            <ul id="choices"
                class="z-20 absolute text-center m-auto border-2 w-100 border-blue-500 rounded-xl overflow-hidden rounded-t-none cursor-pointer">
                @forelse ($ingredients as $i => $ingredient)
                    <li class="py-2 px-5 hover:bg-blue-500 hover:text-white bg-blue-100 @if ($highlightedIndex == $i)
                        bg-blue-500 text-white
                    @endif"
                        wire:click='setIngredientSearch("{{ $ingredient->name }}")' >
                        {{ $ingredient->name }}</li>
                @empty
                    <li class="py-2 px-5 bg-blue-100">
